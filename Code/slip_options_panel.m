...
% if slip_type(1) == 'B' %bulls-eye
% elseif slip_type(1) == 'T' %triangular
% elseif slip_type(1) == 'C' %custom
% end
...
%% building the interface customised for the selected slip type (bulls-eye, triangular, custom)
if sliptype_dd.Value(1) == 'B' || sliptype_dd.Value(1) == 'T'
    %vertical location and extent (not relevant for custom slip)
...
if sliptype_dd.Value(1) == 'T' || sliptype_dd.Value(1) == 'C' %making irrelevant elements invisible
    ver_lbl.Visible = 'off';
    sp_rupt_top.Visible = 'off';
    sp_centre_ver.Visible = 'off';
    sp_rupt_bot.Visible = 'off';
end

%% maximum slip and percentage at surface('B')/slip rate('T')
...
elseif sliptype_dd.Value(1) == 'T' %triangular
    uilabel(slip_fig,'Position',[50 130 140 20],'Text','max. slip rate (mm/a):');
    set_maxSlip = uispinner(slip_fig,'Position',[160 130 60 20],'Step',0.1,'Limits',[0 inf],'Value',settings.value(2),'ValueChangedFcn',update_function,'Tooltip','maximum slip rate at the centre of the fault');
    set_surfSlip = uispinner(slip_fig,'Position',[160 160 60 20],'Visible','off');
elseif sliptype_dd.Value(1) == 'C' %custom
    set_surfSlip = uispinner(slip_fig,'Position',[160 160 60 20],'Visible','off');
    set_maxSlip = uispinner(slip_fig,'Position',[160 160 60 20],'Visible','off');
end

%% moment magnitude button
if sliptype_dd.Value(1) == 'B'
