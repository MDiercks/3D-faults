...
%plot button    
subplot_cb = uicheckbox(fig,'Position',[1040 605 200 20],'Value',true,'Text','Display entire network','Tooltip','Reduce computation time by only plotting the source fault. All faults are modelled.');
exp_geo_cb = uicheckbox(fig,'Position',[1040 625 200 20],'Text','Export fault geometry','Value',false,'HandleVisibility','off');
btn = uibutton(fig,'push','Text','Build 3D faults','Position',[1040, 572, 160, 30],'BackgroundColor',[.8 .3 .3],'FontWeight','bold','ButtonPushedFcn','model_3D_faults','FontSize',18);
slipdist_dd = uidropdown(fig,'Position',[1040 550 160 20],'Items',{'coseismic','interseismic'});
